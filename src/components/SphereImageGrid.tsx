...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/30 bg-white">
          <img
            src={image.src}
            alt={image.alt}
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        <div
          className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-2xl border border-gray-200"
          onClick={(e) => e.stopPropagation()}
          style={{
            animation: 'scaleIn 0.3s ease-out'
          }}
        >
          <div className="relative bg-gray-50 p-10 flex items-center justify-center min-h-[320px]">
            <img
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-3 right-3 w-9 h-9 bg-gray-800 rounded-full text-white flex items-center justify-center hover:bg-gray-900 transition-colors cursor-pointer shadow-lg"
            >
              <X size={18} />
            </button>
          </div>
